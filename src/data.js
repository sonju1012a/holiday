...
// 지방(신위) 단계를 교체합니다. 제주(祭主)인 아버님 기준으로 부모·조부모·
// 증조부모·고조부모 4대 8위를, 사진처럼 부부(考/妣)가 한 장에 나란히 적힌
// 지방 4장으로 모십니다. 남자 조상은 "학생(學生)" 대신 "처사(處士)"를 씁니다.
// -------------------------------------------------------------------
...
const MYFAMILY_ANCESTOR_PAIRS = [
  { label: '부모님', male: '顯考處士府君神位', female: '顯妣孺人達城徐氏神位', pos: [-1.382, 1.6, -2.4] },
  { label: '조부모님', male: '顯祖考處士府君神位', female: '顯祖妣孺人海平尹氏神位', pos: [-0.691, 1.5, -2.4] },
  { label: '증조부모님', male: '顯曾祖考處士府君神位', female: '顯曾祖妣孺人眞城李氏神位', pos: [0.691, 1.6, -2.4] },
  { label: '고조부모님', male: '顯高祖考處士府君神位', female: '顯高祖妣孺人坡平尹氏神位', pos: [1.382, 1.5, -2.4] },
];
...
export const MYFAMILY_SETUP_STEPS = SETUP_STEPS.flatMap((s) => {
  ...
  const pairSteps = MYFAMILY_ANCESTOR_PAIRS.map((a, i) => ({
    ...
    edu: {
      title: `지방(紙榜) — ${a.label}`,
      body: `「${a.male}」 · 「${a.female}」<br/>부부(考/妣)를 한 장에 나란히 적는 것이 우리 집 관례예요.`,
    },
  ...
  const wipaeStep = {
    ...
    edu: {
      title: '위패(位牌) — 단일 제사용',
      body: '「顯考處士府君神位」<br/>지방은 명절 차례가 끝나면 소각하지만, 위패는 기제사(고인 한 분의 제삿날)마다 다시 꺼내 쓰도록 상 위에 남겨두는 것이 다른 점이에요.',
    },
  };
