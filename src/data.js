...
// -------------------------------------------------------------------
// "우리집 차례상" 모드 전용 준비 단계 — 일직 손가 35세손 문중 관례에 맞춰
// 지방(신위) 단계를 교체합니다. 35세손 기준으로 아버님이 생존해 계시므로
// 부모 대는 모시지 않고, 조부모·증조부모·고조부모 3대 6위를 사진처럼
// 부부(考/妣)가 한 장에 나란히 적힌 지방 3장으로 모십니다.
// 남자 조상은 "학생(學生)" 대신 "처사(處士)"를 쓰고, 조상님의 휘(諱, 이름)는
// 적지 않습니다(남자는 府君, 여자는 본관+氏로만 표기).
// -------------------------------------------------------------------
// 병풍이 지그재그로 접혀 있으니, 지방은 패널 정중앙이 아니라 패널과 패널이
// 맞닿는 "모퉁이(접히는 곳)"에 붙입니다.
// (models.js buildByeongpung: 패널 i 중심 x=(i-2.5)*0.6912 → i,i+1 사이 모퉁이 x=(i-2)*0.6912)
const MYFAMILY_ANCESTOR_PAIRS = [
  { label: '조부모님', male: '顯祖考處士府君神位', female: '顯祖妣孺人海平尹氏神位', pos: [-1.382, 1.6, -2.4] },
  { label: '증조부모님', male: '顯曾祖考處士府君神位', female: '顯曾祖妣孺人眞城李氏神位', pos: [0, 1.5, -2.4] },
  { label: '고조부모님', male: '顯高祖考處士府君神位', female: '顯高祖妣孺人邊城谷氏神位', pos: [1.382, 1.6, -2.4] },
];

export const MYFAMILY_SETUP_STEPS = SETUP_STEPS.flatMap((s) => {
  if (s.id !== 'jibang') return [s];
  const pairSteps = MYFAMILY_ANCESTOR_PAIRS.map((a, i) => ({
    id: `jibang_mf_${i}`,
    name: `${a.label} 지방 모시기`,
    emoji: '📜',
    model: `jibang_mf_${i}`,
    ...
    edu: {
      title: `지방(紙榜) — ${a.label}`,
      body: `「${a.male}」 · 「${a.female}」<br/>부부(考/妣)를 한 장에 나란히 적는 것이 우리 집 관례예요. 조상님의 이름(휘·諱)은 적지 않고, 할아버님은 「處士府君」, 할머님은 「孺人 + 본관 + 氏」로만 씁니다.`,
    },
    shop: i === 0 ? 'jibang' : undefined,
  }));
  const wipaeStep = {
    id: 'wipae',
    name: '위패 모시기',
    emoji: '🪧',
    model: 'wipae',
    pos: [-2.2, 1.09, -1.5], // 상 윗면(TABLE_TOP_Y=0.82) 위, 왼쪽 뒤(병풍 쪽) 구석
    zone: 0.8,
    guide: '지방과 달리 <b>위패</b>는 병풍이 아니라 <b>차례상 위</b>에 놓습니다. 기제사(단일 제사) 때 쓰는 위패부터 먼저 모셔요.',
    edu: {
      title: '위패(位牌) — 단일 제사용',
      body: '「顯高祖考處士府君神位」 · 「顯高祖妣孺人邊城谷氏神位」<br/>우리 집 위패에는 고조부모님 내외를 한 장에 나란히 모시고, 두 분 모두 이름(휘·諱)은 쓰지 않아요. 벼슬을 하지 않은 선비이시라 할아버님은 「處士府君」, 할머님은 그 아내를 뜻하는 「孺人」에 본관·성씨(邊城谷氏)만 붙입니다.<br/>지방은 명절 차례가 끝나면 소각하지만, 위패는 기제사(고인 한 분의 제삿날)마다 다시 꺼내 쓰도록 상 위에 남겨두는 것이 다른 점이에요.',
    },
  };
  return [wipaeStep, ...pairSteps];
});
...
export const FOOD_ROWS = [
  {
    ...
    guide: '신위(지방)에서 가장 가까운 <b>1열</b>부터 놓아요. 시접과 술잔은 <b>가운데</b>, 메(밥)는 <b>서쪽(왼쪽)</b>, 국은 <b>동쪽(오른쪽)</b>!',
    rules: ['order', 'banseo'],
    z: -0.95,
    slots: [
      { id: 'me',     x: -1.5, name: '메' },
      { id: 'sijeop', x: -0.5, name: '시접' },
      { id: 'jan',    x: 0.5,  name: '술잔' },
      { id: 'guk',    x: 1.5,  name: '국' },
    ],
    items: ['me', 'tteokguk', 'sijeop', 'jan', 'makgeolli', 'guk'],
  },
  {
    row: 2,
    title: '2열 — 적과 전',
...
// -------------------------------------------------------------------
// "우리집 차례상" 모드 — 일직 손가 35세손 실제 상 그대로. 자유 배치(free),
// 정석 규칙(어동육서 등) 대신 우리 집 가풍을 따릅니다.
// -------------------------------------------------------------------
export const MYFAMILY_ROWS = [
  {
    row: 1,
    title: '1열 — 시접·잔·술밥·국',
    guide: '<b>우리집 차례상</b>이에요. 시접·술잔·술밥·국을 <b>빈 자리 아무 곳에나</b> 편하게 놓으세요.',
    rules: [],
    z: -1.05,
    slots: [
      { id: 'mf1_1', x: -1.5 },
    ...
    items: ['sijeop', 'mf_jan', 'mf_bap', 'mf_guk'],
  },
  {
    row: 2,
    title: '2열 — 탕 (문어·닭·소고기조림)',
    guide: '<b>2열은 탕!</b> 우리 집에서 탕은 국물이 아니라 <b>간장에 조린 요리</b>를 뜻해요.',
    rules: [],
    z: -0.6,
    slots: [
      { id: 'mf2_1', x: -1.3 },
    ...
    items: ['mf_muneotang', 'mf_dak', 'mf_sogogitang'],
  },
  {
    row: 3,
    title: '3열 — 포·수육·생선·계란',
    guide: '<b>3열</b>은 오징어·동태포·수육·조기·계란이에요.',
    rules: [],
    z: -0.15,
    slots: [
      { id: 'mf3_1', x: -1.9 },
    ...
    items: ['mf_ojingeo', 'mf_dongtaepo', 'mf_suyuk', 'mf_jogi', 'mf_gyeran'],
  },
  {
    row: 4,
    title: '4열 — 전·튀김',
    guide: '<b>4열</b>은 전과 튀김! 햄전·오징어튀김·동태전·새우튀김·두부전·육전·부추전까지 일곱 가지예요.',
    rules: [],
    z: 0.3,
    slots: [
      { id: 'mf4_1', x: -2.4 },
    ...
    items: ['mf_hamjeon', 'mf_ojingeotwigim', 'dongtaejeon', 'mf_saewootwigim', 'mf_dubujeon', 'yukjeon', 'mf_buchujeon'],
  },
  {
    row: 5,
    title: '5열 — 나물 여섯 가지',
    guide: '<b>5열</b>은 나물! 무·콩나물·숙주·시금치·도라지·고사리를 올려요.',
    rules: [],
    z: 0.75,
    slots: [
      { id: 'mf5_1', x: -2.0 },
    ...
    items: ['mf_mu', 'mf_kongnamul', 'mf_sukju', 'mf_sigeumchi', 'mf_doraji', 'mf_gosari'],
  },
  {
    row: 6,
    title: '6열 — 과일과 한과',
    guide: '마지막 <b>6열</b>은 과일! 대추·밤·곶감·배·사과를 올려요.',
    rules: [],
    z: 1.2,
    slots: [
      { id: 'mf6_1', x: -2.15 },
    ...
    items: ['daechu', 'bam', 'gotgam', 'bae', 'sagwa'],
  },
];
...
export const MODES = {
  ...
  myfamily: {
    key: 'myfamily',
    label: '일직 손가 35세손 차례상',
    rows: MYFAMILY_ROWS,
    free: true,
    setupSteps: MYFAMILY_SETUP_STEPS,
    summary: `
      ① 일직 손가 35세손 — 실제 우리 집 차례상을 그대로 재현했어요<br/>
      ② 아버님 기준 <b>부모·조부모·증조부모·고조부모 4대 8위</b>를 함께 모셔요<br/>
      ③ 남자 조상은 「학생(學生)」 대신 <b>「처사(處士)」</b>를 씁니다<br/>
      ④ 우리 집에서 "탕"은 국물이 아니라 <b>간장에 조린 요리</b>를 뜻해요<br/>
      ⑤ 정석 규칙보다 <b>우리 집 가풍 그대로</b>가 기준입니다`,
  },
};
